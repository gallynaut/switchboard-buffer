...
import {getOrCreateAssociatedTokenAccount} from "@solana/spl-token2";
import {Buffer} from "buffer";
import {promiseWithTimeout, sleep, SwitchboardTestContext} from "@switchboard-xyz/sbv2-utils";
import {
    AggregatorAccount,
...
const network = () => {
    if (process.env.SOLANA_ENV === "devnet" || process.env.SOLANA_ENV === "testnet" || process.env.SOLANA_ENV === "mainnet-beta") {
...
const airdrop = async ({provider, pk}: { provider: AnchorProvider, pk: PublicKey }) => {
...
export type Runner = { wallet: Wallet; provider: AnchorProvider; program: Program<SwitchboardBuffer>; user: Keypair; programId: PublicKey, aggregatorAccount: AggregatorAccount, aggregatorDataAccount: any, aggregatorPublicKey: PublicKey, crankData?: any, crankAccount?: CrankAccount, queueData?: any, queueAccount: OracleQueueAccount, switchboardToken?: PublicKey }
...
export const setupTest = async ({
    ...
    try {
        ...
        if (turnCrankAction) {
            ...
            await program.methods.turnCrank()
                .accounts({
        ...
        if (pushBufferAction) {
            ...
            let ix = await program.methods
                ...
                .signers([user]);
            console.log("data length", (await ix.instruction()).keys.map(f => f.pubkey.toString()), await ix.transaction())
            await ix.rpc(provider.connection)
                .catch((err: AnchorError) => {
                    console.log(err);
                });
        }
